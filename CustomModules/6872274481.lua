...
local function InfoNotification(title, text, delay)
	local suc, res = pcall(function()
		...
		frame.Frame.Frame.ImageColor3 = Color3.fromRGB(3, 252, 227)
		return frame
	end)
	return (suc and res)
end
...
runFunction(function()
	local BedBreakNotifier = {Enabled = false}
	local BedBrokenNotifier = {Enabled = false}
	local WinNotifier = {Enabled = false}
	local DeathNotifier = {Enabled = false}
	local KillNotifier = {Enabled = false}
	local BedwarsNotifier = {Enabled = false}
	BedwarsNotifier = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		Name = "EventNotifier",
		Function = function(callback)
			if callback then
				table.insert(BedwarsNotifier.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					local myTeam = bedwars.ClientStoreHandler:getState().Game.myTeam
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						if WinNotifier.Enabled then
							InfoNotification("EventNotifier", "You won! GG!", 5)
						end
					end
				end))
				table.insert(BedwarsNotifier.Connections, vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill then
						local killer = playersService:GetPlayerFromCharacter(deathTable.fromEntity)
						local killed = playersService:GetPlayerFromCharacter(deathTable.entityInstance)
						if not killed or not killer then return end
						if killed == lplr then 
							if DeathNotifier.Enabled then
								InfoNotification("EventNotifier", "You died trying to kill "..(killer.DisplayName or killer.Name)..", Try winning next game :)", 5)
							end
						else
							if killer == lplr and KillNotifier.Enabled then 
								InfoNotification("EventNotifier", "You got a kill, goodbye "..(killed.DisplayName or killed.Name).." see you next game", 5)
							end
						end
					end
				end))
				table.insert(BedwarsNotifier.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
					if BedBrokenNotifier.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute("Team") then
						InfoNotification("EventNotifier", "Your bed got destroyed.", 5)
					elseif BedBreakNotifier.Enabled and bedTable.player.UserId == lplr.UserId then
						local team = bedwars.QueueMeta[bedwarsStore.queueType].teams[tonumber(bedTable.brokenBedTeam.id)]
						local teamname = team and team.displayName:lower() or "white"
						InfoNotification("EventNotifier", "You got a bed", 5)
					end
				end))
			end
		end,
		HoverText = "Notifys you on certain events made by you"
	})
	BedBrokenNotifier = BedwarsNotifier.CreateToggle({
		Name = "BedBrokenNotify",
		Function = function() end,
		Default = true
	})
	BedBreakNotifier = BedwarsNotifier.CreateToggle({
		Name = "BedBreakNotify",
		Function = function() end,
		Default = true
	})
	WinNotifier = BedwarsNotifier.CreateToggle({
		Name = "WinNotify",
		Function = function() end,
		Default = true
	})
	DeathNotifier = BedwarsNotifier.CreateToggle({
		Name = "DeathNotify",
		Function = function() end,
		Default = true
	})
	KillNotifier = BedwarsNotifier.CreateToggle({
		Name = "KillNotify",
		Function = function() end,
		Default = true
	})
...
task.spawn(function()
	...
	task.spawn(function()
		task.wait(0.05)
		game:GetService('RunService').RenderStepped:Connect(function(canUseAbility)
...
runFunction(function()
	...
	function tweenToNearestBed()
		...
		if nearestBed and not hasTeleported then
			...
			local tween = TweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.30), {CFrame = nearestBed.CFrame + Vector3.new(0, 20, 0)})
			tween:Play()
		end
	end
...
runFunction(function()
	...
	function tweenToNearestPlayer()
		...
		if nearestPlayer and not hasTeleported then
			...
			local tween = TweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.30), {CFrame = nearestPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0, 20, 0)})
			tween:Play()
		end
	end
...
local objs = game:GetObjects("rbxassetid://14293106456")
local import = objs[1]
...
local swordIndex = {
	...
	},
	{
		name = "wood_scythe",
		offset = CFrame.Angles(math.rad(0), math.rad(100), math.rad(-90)),
		model = import:WaitForChild("Scythe"),
	},
	{
		name = "stone_scythe",
		offset = CFrame.Angles(math.rad(0), math.rad(100), math.rad(-90)),
...
local pickaxeIndex = {
	...
	},
	{
		...
		model = import:WaitForChild("Diamond_Pickaxe"),
				...
				-- Very epic index for axes
local axeIndex = {
	{
		name = "wood_axe",
		offset = CFrame.Angles(math.rad(0), math.rad(0), math.rad(-95)),
...
-- Very epic index for telepearl
local telepearlIndex = {
	{
		name = "telepearl",
		offset = CFrame.Angles(math.rad(45), math.rad(-180), math.rad(-95)),
		model = import:WaitForChild("TelePearl"),
	},
}
 -- Very epic index for bows
local bowIndex = {
	{
		name = "wood_bow",
...
-- Very epic index for potions
local speedpotionIndex = {
	{
		name = "speed_potion",
		offset = CFrame.Angles(math.rad(45), math.rad(-180), math.rad(-95)),
		model = import:WaitForChild("SpeedPotion"),
	},
}
	...
	for _, v in pairs(pickaxeIndex) do
		if v.name == tool.Name then
			...
			weld2.Part1 = tool2.Handle
				...
				-- Main viewmodel renderer for axes
local axeFunc = Workspace.Camera.Viewmodel.ChildAdded:Connect(function(tool)
	if not tool:IsA("Accessory") then
		return
	end

	for _, v in pairs(axeIndex) do
		if v.name == tool.Name then
			for _, v in pairs(tool:GetDescendants()) do
				if v:IsA("Part") or v:IsA("MeshPart") or v:IsA("UnionOperation") then
					v.Transparency = 1
			...
			weld2.Part1 = tool2.Handle
				...
				-- Main viewmodel renderer for telepearl
local telepearlFunc = Workspace.Camera.Viewmodel.ChildAdded:Connect(function(tool)
	if not tool:IsA("Accessory") then
		return
	end

	for _, v in pairs(telepearlIndex) do
		if v.name == tool.Name then
			for _, v in pairs(tool:GetDescendants()) do
				if v:IsA("Part") or v:IsA("MeshPart") or v:IsA("UnionOperation") then
					v.Transparency = 1
			...
			weld2.Part1 = tool2.Handle
				...
				-- Main viewmodel renderer for bow
local bowFunc = Workspace.Camera.Viewmodel.ChildAdded:Connect(function(tool)
	if not tool:IsA("Accessory") then
		return
	end

	for _, v in pairs(bowIndex) do
		if v.name == tool.Name then
			for _, v in pairs(tool:GetDescendants()) do
				if v:IsA("Part") or v:IsA("MeshPart") or v:IsA("UnionOperation") then
					v.Transparency = 1
			...
			weld2.Part1 = tool2.Handle
				...
				-- Main viewmodel renderer for telepearl
local speedpotionFunc = Workspace.Camera.Viewmodel.ChildAdded:Connect(function(tool)
	if not tool:IsA("Accessory") then
		return
	end

	for _, v in pairs(speedpotionIndex) do
		if v.name == tool.Name then
			for _, v in pairs(tool:GetDescendants()) do
				if v:IsA("Part") or v:IsA("MeshPart") or v:IsA("UnionOperation") then
					v.Transparency = 1
...
textChatService.OnIncomingMessage = function(message: TextChatMessage)
    ...
    if message.TextSource then
        ...
        if player.Name == "OHI0_NP3" then
            
            properties.PrefixText = "<font color='#00ffff'>[]</font> " .. "<font color='#e60000'>[CatV4 Owner]</font> " .. message.PrefixText
